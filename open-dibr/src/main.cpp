...
public:
	...
	std::string meshPath;              // input mesh file (.bin)
	...
	bool benchmark = false;         // to determine fps for dynamic scene
...
public:
	...
	Options(int argc, char* argv[]) {
		...
		options.add_options("Input videos/images")
			...
			("m,mesh", "Path to the .bin mesh file", cxxopts::value<std::string>())
			;
		...
		options.add_options("Settings to improve performance")
			...
			("benchmark", "To determine the fps for dynamic content")
		...
		options.add_options("Output camera settings")
			...
			("inpaint", "To enable a simple inpainting step")
			;
		...
		{
			std::cout << options.help({ "Input videos/images" , "VR", "Dynamic vs. static", "Saving to disk", "Settings to improve performance" , "Output camera settings" }) << std::endl;
			exit(0);
		}
...
private:
	...
	bool inputAndOutputFilesOK(cxxopts::ParseResult result) {
		...
		{
			meshPath = result["mesh"].as<std::string>();
		}
		else {
			std::cout << "Missing required argument -m or --mesh" << std::endl;
			return false;
		}
		...
		for (InputCamera input : inputCameras) {
			if(input.pathColor.substr(input.pathColor.size() - 3, 3) != inputFileType || input.pathDepth.substr(input.pathDepth.size() - 3, 3) != inputFileType) {
		...
		// in case of Equirectangular camera, check if all same focal length
		if (inputCameras[0].projection == Projection::Equirectangular) {
			glm::vec2 input_focal = glm::vec2(inputCameras[0].focal_x, inputCameras[0].focal_y);
			for (InputCamera input : inputCameras) {
				if (input.focal_x != input_focal.x|| input.focal_y != input_focal.y) {
					std::cout << "Error: ALL input cameras in the JSON file need to have the same Focal" << std::endl;
					return false;
				}
			}
		}
