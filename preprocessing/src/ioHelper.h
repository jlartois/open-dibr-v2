...
bool readInputJson(std::string inputJsonPath, std::string directory, /*out*/std::vector<InputCamera>& inputCameras, int& nrFrames) {
	std::ifstream file;
	...
	if (!j.contains("Number_of_frames")) {
		std::cout << "Error: the input JSON file should contain a key \"Number_of_frames\" with the number of frames per video." << std::endl;
		return false;
	}
	nrFrames = j["Number_of_frames"];
